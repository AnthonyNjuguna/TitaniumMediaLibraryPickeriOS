...
-(id)initWithReuseIdentifier:(NSString*)_identifier numberOfThumbnails:(NSInteger)numberOfThumbnails {
	if((self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:_identifier])) {
        ...
        for (int i =0; i < numberOfThumbnails; ++i) {
            BMMLPAssetThumbnailView *assetView = [[BMMLPAssetThumbnailView alloc] initWithFrame:CGRectMake(0,0,THUMBNAIL_DIMENSION,THUMBNAIL_DIMENSION)];
...
-(void)layoutSubviews {
	...
	for(BMMLPAssetThumbnailView *assetView in self.thumbnailViews) {		
